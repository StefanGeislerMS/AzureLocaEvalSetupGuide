...
## Create, install and prepare the cluster noce VMs
...
* In the next step you can chose to download a “preview” version of the install media. We use this version, because it supports easy, graphical ARC onboarding.  
![Image](/img/007.png)
...
*	We’ll use the “CreateAzureLocalClusterNodeVM.ps1” PowerShell Script to create the VMs
...
* Open a PowerShell commandline with administrative permissions and run the script. In case everything works fine, you will find two new VMs in your Hyper-V Manager. You can open the VM properties and double verify the following properties:
...
*	In a next step, we will configure networking and assign static IPs for both VMs. For this purpose, we will use the <code>PrepVMNetwork.ps1</code> script. In this checklist we use 10.0.0.220-10.0.0.227 as IP address for the cluster nodes, and re reserve 10.0.0.180-10.0.0.185 for cluster appliances and internal management purposes. Gateway IP address is 10.0.0.1, DNS is 10.0.0.2. If your IP addresses are differnet, you need to adjust them in the <code>PrepVMNetwork.ps1</code> script accordingly. Edit the script, in case you need to use different IP addresses.
*	The script will set the MAC addresses on the physical interfaces and rename them NIC1-NIC4. After that it’ll disable DHCP and assign static IP-Addresses. Finally, it’ll enable Hyper-V on the VMs and it configures time server settings.
*	Run the script on a workstation or server located in the on-premises network – don’t run the script on the cluster nodes itself.
...
* To onboard both VMs to azure ARC connect to each VM using a browser [http://ALNode1.local](http://ALNode1.local) and [http://ALNode2.local](http://ALNode2.local)
...
*	In the Azure portal search for ARC and select “Azure Local” – click “Deploy instance” in “Depoly Azure Local”
![Image](/img/026.png)  

*	Select the right resource group for you Azure Local Cluster and specify a new Instance name. Press “Add machines and select the two nodes., that we added to ARC before. Press the “Validate selected machines” button to check if the machines are ready for install.  
![Image](/img/027.png)  
...
![Image](/img/028.png)  Chose a valid name for the new Key vault an press the Create Button  
![Image](/img/029.png)  

*	Once the new key vault is created, the portal will complain, that access permissions are not configured correctly. Click the “Grant Key Vault Permissions” Button to fix this.  
![Image](/img/030.png)

* After that press the “Next” button to continue. Chose to create a new configuration and continue.  
![Image](/img/031.png) 
...
* Next enter the Domain for your on-premises Active Directory and the path to the OU for Azure Local. Also enter the users to be used for the Cluster install and the local Administrator credentials.  
![Image](/img/035.png)  Continue to the next step.

*	Accept the recommended security settings and continue with the next step.  
![Image](/img/036.png)
...
The following steps are required to configure the cluster for on-premises VM deployment. First, we will create an image to be used for VM deployment. Next, we will set up a logical network for the new VMs. Finally, we will create a VM.

## Activate Azure Hybrid Benefits

*	Make sure you enable the “Hybrid Benefits” for the new Azure Local cluster nodes. This will allow you to utilize Windows Admin Center to administrate the cluster node easier.  
...
*	Select one machine after the other and enable Azure Benefits.  
![Image](/img/044.png)  
...
*	The deployment of an image to Azure Local may take anywhere from several minutes to several hours, depending on the network bandwidth between Azure Local and Azure. You can monitor the status of the deployment in the Azure portal.  ![Image](/img/047.png)  
...
*	When Image and logical network creation finished, we can launch creation of a 1st VM on the cluster. To create a new logical network, open Azure Local in the portal, select the cluster instance and under Resources create a new Virtual Machine.  
![Image](/img/054.png)
...
*	Press next, skip creation of a data disk and press next again to configure networking. Add a network interfact to the VM.  
![Image](/img/057.png)
...
![Image](/img/061.png)
Deployment of the Windows Admin Center tank take up to multiple minutes.

*	Once deployment is done, you can connect to Windows Admin Center  
![Image](/img/062.png)  

* Once connected you can use Windows Admin Center to configure cluster configuration.  
![Image](/img/063.png)  
![Image](/img/064.png)  
![Image](/img/065.png)
